...
export function Therapy({ id, doctorId, patientId }: Props) {
  ...
  const { data } = useTreatment({
    ...
    // status,
    category:
      id === ReceptionTableEnum.ALL ? '' : ReversedReceptionTableEnum[id] ?? '',
  });
    ...
    return (
      <div className={s.empty}>
        ...
        <Button
          ...
          onClick={() =>
            navigate(PATH_PAGE.template.tab(id, patientId, doctorId))
          }
        >
          <AiOutlinePlusCircle />
          Создать прием
        ...
        <div key={treatment.id} className={s.card}>
          <div className={classNames(s.blockInfo, s.headInfo)}>
            ...
            <div>
              ...
              <span className={s.status}>
                {treatment.status === 'ACTIVE' ? ' В процессе' : 'Закончено'}
              </span>
            </div>
          </div>
          ...
          {treatment.subTreatments.map((subTreatment) => {
            ...
            return (
              <div
                key={subTreatment.id}
                className={classNames(s.blockInfo, s.subTreatment)}
                onClick={() =>
                  navigate(
                    PATH_PAGE.reception.viewRecord(
                      patientId!,
                      treatment.id.toString(),
                      subTreatment.id.toString(),
                    ),
                  )
                }
                onKeyDown={() => false}
              ...
              >
                <div className={classNames(s.info, s.dateTime)}>
                  ...
                  | {startTime.format('DD.MM.YYYY')} {startTime.format('dddd')}
                </div>

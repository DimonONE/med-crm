...
export function PatientInfo({ patientId, backButtonLink }: PatientInfoProps) {
  ...
  const [selectRecord, setSelectRecord] = useState<Api.RecordEntityDto | null>(
    null,
  );
  ...
  const hasRecords = recordsPatient?.length !== 0;

  const dateOfBirth = dayjs().diff(dayjs(data?.dateOfBirth), 'year');
  const priceServices = useMemo(
    () =>
      recordsPatient
        ? recordsPatient.reduce((accRecord, nextRecord) => {
            const servicePrices = nextRecord.servicePrices.reduce(
    ...
    <div className={classNames(s.root, 'container')}>
      ...
      <div className={classNames('d-flex', s.blockInfo)}>
        ...
        <div className={s.userInfo}>
          ...
          <div className={s.textInfo}>
            {dayjs(data.dateOfBirth).format('DD.MM.YYYY')} | {dateOfBirth}{' '}
            год(а)
          </div>
          <div className={s.textInfo}>Код клиента: {data.id}</div>
          <div className={classNames(s.textInfo, s.date)}>
            Рег. {dayjs(data.createdAt).format('DD.MM.YYYY')}
          </div>
        </div>
        ...
        <div className={s.userInfo}>
          ...
          <div className={classNames(s.textInfo, s.passportNumber)}>
        ...
        <div key={record.id} className={s.blockInfo}>
          <div className={s.recording}>
            ...
            <div className={s.contentInfo}>
              <div>
                {`${dayjs(record.startTime).format('DD.MM.YYYY')} | ${dayjs(
                  record.startTime,
                ).format('HH:mm dddd')}`}{' '}
              </div>
              <div>
          ...
          <div className={s.recording}>
            ...
            <div className={s.contentInfo}>
              {record.servicePrices.map(({ price, name }) => (
                <div key={`${name}-${price}`} className={s.nameDisease}>
                  {name}
                </div>
              ))}
            </div>
          ...
          <div className="d-flex">
            <Button
              className={classNames(s.button, s.buttonSuccess)}
              onClick={() => false}
            >
              В оплату
            </Button>
            ...
            <Button
              className={classNames(s.button, s.buttonAttendance)}
              onClick={() => false}
            >
              Неявка
            </Button>
            ...
            <Button
              className={classNames(s.button, s.buttonCancel)}
              onClick={() => false}
            >
              Отмена
            </Button>
      ...
      <Tooltip
        title={!hasRecords ? 'Нет доступных записей' : ''}
        placement="left"
      >
        <span>
          <NavLink
            to={
              !hasRecords
                ? '#'
                : PATH_PAGE.reception.info(patientId, userInfo?.id!)
            }
            className={`${s.tableInfo} ${!hasRecords ? s.disabled : ''}`}
            onClick={(e) => !hasRecords && e.preventDefault()}
          >
            Таблица приема
          </NavLink>
        </span>
      ...
      <div className={s.tableInfo}>История платажей</div>

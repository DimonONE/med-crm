...
export function DetailTemplatePage() {
  ...
  const titleBackLink = checkUserRole('superAdmin')
    ? PATH_PAGE.root
    : PATH_PAGE.patients.records;
  ...
  const onNavigate = (templateId: number) => {
    const createData = {
      ...
      status: 'in progress',
      category:
        id === ReceptionTableEnum.ALL
          ? ''
          : ReversedReceptionTableEnum[id!] ?? '',
    };

    mutate(createData, {
      onSuccess: (data) => {
        navigate(
          PATH_PAGE.reception.create(
            patientId!,
            doctorId!,
            templateId.toString(),
            data.id.toString(),
  ...
  const getContent = (key: 'templates' | undefined) => {
    switch (key) {
      case 'templates':
        return (
          <Template id={id ?? ReceptionTableEnum.ALL} onNavigate={onNavigate} />
        );
  ...
  return (
    <>
      ...
      <div className={s.root}>
        <nav className={s.navigate}>
          ...
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === undefined || id === 'undefined',
            })}
            to={PATH_PAGE.template.tab('undefined', patientId, doctorId)}
          >
            ВСЕ
          </NavLink>
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === ReceptionTableEnum.PERIODONTICS,
            })}
            to={PATH_PAGE.template.tab(
              ReceptionTableEnum.PERIODONTICS,
              patientId,
              doctorId,
            )}
          >
          ...
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === ReceptionTableEnum.THERAPY,
            })}
            to={PATH_PAGE.template.tab(
              ReceptionTableEnum.THERAPY,
              patientId,
              doctorId,
            )}
          >
          ...
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === ReceptionTableEnum.SURGERY,
            })}
            to={PATH_PAGE.template.tab(
              ReceptionTableEnum.SURGERY,
              patientId,
              doctorId,
            )}
          >
          ...
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === ReceptionTableEnum.ORTHOPEDICS,
            })}
            to={PATH_PAGE.template.tab(
              ReceptionTableEnum.ORTHOPEDICS,
              patientId,
              doctorId,
            )}
          >
          ...
          <NavLink
            className={classNames(s.tab, {
              [s.active]: id === ReceptionTableEnum.OTHER,
            })}
            to={PATH_PAGE.template.tab(
              ReceptionTableEnum.OTHER,
              patientId,
              doctorId,
            )}
          >
        ...
        <div className={s.container}>{getContent('templates')}</div>
        {checkUserRole('superAdmin') && (
          <Button
            className="fixed-button"
            style={{ background: '#229CE1' }}

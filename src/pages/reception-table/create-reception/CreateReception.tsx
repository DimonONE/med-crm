...
import { TemplateStatus, useTemplateGetOne, useTreatmentAnswerCreate, useTreatmentId } from '~features/draggable-list';
...
export function CreateReceptionPage() {
  ...
  const [template, setTemplate] = useState<Api.SubTemplateEntityDto | undefined>(undefined);
  ...
  const [subTemplateId, setSubTemplateId] = useState<string | number | undefined>(undefined);
  ...
  console.log('dataTreatment', dataTreatment);
  ...
  const subTemplateOptions = useMemo(() => dataTemplate?.subTemplates.map(({ id, name }) => ({
    value: id,
    label: name,
  })) ?? [], [dataTemplate]);

  const onCreate = () => {
    const selectTemplate = dataTemplate?.subTemplates.find(({ id }) => id === subTemplateId);
  ...
  const onSave = () => {
    ...
    if (Object.values(visitSummary).some(value => value === '')) {
      toast('Краткое резюме посещения обязательно к заполнению!', { type: 'warning' });
      return;
    }

    const createData = {
      recordId: 9, // из аписи пациента http://localhost:3000/patients/records/afc886e0-2f31-4d21-92c1-0f99f2c4dd33
  ...
  const handleVisitSummary = (key: 'completed' | 'comment' | 'plan' | 'files', value: string | FileValues) => {
      ...
      <div className={s.container}>
        <div className={s.statusBlock}>
          ...
          <SelectField
            ...
            className={classNames('form-input buttons-header', s.selectTemplate)}
            selectNavigate
        ...
        <div className={s.submitBlock}>
          ...
          <Button className={classNames(s.selectTemplate, s.submit)} onClick={onCreate}>Создать прием</Button>
        </div>
        ...
        ))}
        {
          template && (<>
            <div className={classNames(s.draggable)}>
              ...
              <div className={s.visitSummary}>
                ...
                <UnderlineText
                  ...
                  name=''
                  onChange={(event) => typeof event === 'object' ? handleVisitSummary('plan', event.target.value) : false}
              ...
              <div className={s.visitSummary}>
                ...
                <UnderlineText
                  ...
                  name=''
                  onChange={(event) => typeof event === 'object' ? handleVisitSummary('completed', event.target.value) : false}
                  className={s.visitSummaryText}
                />
              </div>
              <div className={s.visitSummary}>
                ...
                <UnderlineText
                  ...
                  name=''
                  onChange={(event) => typeof event === 'object' ? handleVisitSummary('comment', event.target.value) : false}
                  className={s.visitSummaryText}
                />
              </div>
            ...
            <Button className={s.save} onClick={onSave}>Сохранить</Button>
          </>)
        }
      </div>
    </div>
